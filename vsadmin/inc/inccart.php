<?php /* 100markets.com - START - Skip cart summary page */
$sSQL = "SELECT cartID FROM cart WHERE cartCompleted=0 AND cartSessionID='". session_id()."'";
$result = mysql_query($sSQL) or print(mysql_error());
if(!isset($_POST['mode']) && mysql_num_rows($result) > 0) $_POST['mode'] = 'checkout';
mysql_free_result($result);
/* 100markets.com - END - Skip cart summary page */ ?>
<?php
...
function initshippingmethods(){
	...
	if($shipType==2 || $shipType==5){ // Weight / Price based shipping
		...
		elseif($shipType==7){
			...
			if($international=='') $sSQL .= ' AND uspsMethod<>' . ($commerciallocpost=='Y' ? "'GROUNDHOMEDELIVERY'" : "'FEDEXGROUND'");
		}
		$result = mysql_query($sSQL) or print(mysql_error());
		if(mysql_num_rows($result) > 0){
			while($rs = mysql_fetch_row($result))
...
if(@$_GET["token"] != ''){
	if(getpayprovdetails(18,$username,$password,$data3,$demomode,$ppmethod)){
	...
	$sXML = ppsoapheader($username, $password, $signature) .
		...
		'    <Custom>' . (@$_SESSION['clientID'] != '' ? 'cid' . $_SESSION['clientID'] : 'sid' . $thesessionid) . ':' . @$_POST['PARTNER'] . '</Custom>' .
...
<?php
	...
	writehiddenvar('PARTNER', @$_POST['PARTNER']); ?>
...
?></script><?php
	...
	if(! $paypalexpress){
		...
		if(@$enableclientlogin && @$_SESSION['clientID'] != ''){
			$sSQL = "SELECT clEmail FROM customerlogin WHERE clID=" . $_SESSION['clientID'];
			$result = mysql_query($sSQL) or print(mysql_error());
			if($rs = mysql_fetch_assoc($result)) $ordEmail = trim($rs['clEmail']); else print 'Could not find customer.<br />';
		}else
...
<?php
	...
	if($requiressl || @$pathtossl != ''){
		if(@$pathtossl != ''){
			...
			$gcallbackpath = $pathtossl . 'vsadmin/gcallback.php';
		}else{
			$cartpath = str_replace('http:','https:',$storeurl) . 'cart.php';
			$gcallbackpath = str_replace('http:','https:',$storeurl) . 'vsadmin/gcallback.php';
		}
	}else{
		$cartpath='cart.php';
		$gcallbackpath= $storeurl . 'vsadmin/gcallback.php';
	}
	$loginerror='';
	if(@$_GET['mode']=='logout'){
...
<?php		}
			if($wantzipselector){ ?>
              <tr height="30">
				...
				<td class="cobll" bgcolor="#FFFFFF" align="left" colspan="2"><input type="text" name="zip" size="8" value="<?php print str_replace('"','&quot;',$destZip)?>"></td>
			  </tr>
<?php		}
		}
		if(@$showtaxinclusive){
