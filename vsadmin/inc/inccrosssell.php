	...
	for($csindex=0; $csindex < count($crosssellactionarr); $csindex++){
		...
		if($crosssellaction=='alsobought'){ // Those who bought what's in your cart also bought.
			...
			if($prodlist != '' && $sessionlist != ''){
				$sSQL = "SELECT cartProdID FROM cart WHERE cartSessionID IN (" . $sessionlist . ") AND NOT (cartProdID IN (" . $prodlist . ")) ORDER BY cartDateAdded DESC LIMIT 0," . $numberofproducts;
				// print sSQL . "<br>"
				$addcomma='';
				$relatedlist='';
				$result = mysql_query($sSQL) or print(mysql_error());
		...
		if($relatedlist != ''){
			...
			$sSQL = "SELECT pId," . getlangid('pName',1) . ",pImage," . $WSP . "pPrice,pListPrice,pSection,pSell,pStockByOpts,pStaticPage,pInStock,pExemptions,pLargeImage,pTax,'' AS " . getlangid('pDescription',2) . "," . getlangid('pLongDescription',4) . " FROM products WHERE pId IN (" . $prodlist . ")";
