...
function trimoldcartitems($cartitemsdel){
	...
	if($delAfter != 0) $sSQL .= "OR (cartClientID=0 AND cartDateAdded<'" . date("Y-m-d H:i:s", $thedate-($delAfter*60*60*24)) . "') ";
	if($delSavedCartAfter != 0) $sSQL .= "OR (cartDateAdded<'" . date("Y-m-d H:i:s", $thedate-($delSavedCartAfter*60*60*24)) . "') ";
	$sSQL .= ')';
	$addcomma='';
	$result = mysql_query($sSQL) or print(mysql_error());
	...
	if($delAfter != 0) mysql_query("DELETE FROM orders WHERE ordAuthNumber='' AND ordDate<'" . date("Y-m-d H:i:s", $thedate-($delAfter*60*60*24)) . "' AND ordStatus=2");
}
?>